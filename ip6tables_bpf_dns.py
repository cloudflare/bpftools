...
#
#    ./%(name)s 2a00:1450:4009:803::1008/128
#
...
if [ "$*" == "--delete" ]; then
    ...
    ${IPSET} -exist create %(name)s hash:net family inet6
    while [ "$*" != "" ]; do
...
bytecode = subprocess.check_output(['./bpf_dns.py', '-o0', '-6'] + domains)
assembly = subprocess.check_output(['./bpf_dns.py', '-o0', '-6', '-s'] + domains)
...
name = 'bpf_dns_ip6_' + '_'.join(name_parts)
