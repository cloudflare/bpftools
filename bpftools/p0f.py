...
class P0fBPF(object):
    ...
    def parse_sig(self):
        ...
        if self.win_size != "*" and not re.match(
            "^((mss\*)|(mtu\*)|(%))?(\d+)$", self.win_size
        ):
    ...
    def build_olen(self):
        if self.ver == "4" and self.olen != "*":
            hl = self.ip_field("hl")
            self.steps.append(
                [hl, "==", 5 + int(self.olen), "IP options len == %s" % self.olen]
            )
    ...
    def build_win_size(self):
        if self.win_size != "*":
            ...
            if self.win_size_type == "mss_mult":
                ...
                self.steps.append(
                    [win_size, "==", mss_mult, "win size == mss * %s" % self.win_size]
                )
                ...
                self.steps.append(
                    [const_mul, "==", 0, "win size == x * %s" % self.win_size]
                )
            else:
                self.steps.append(
                    [win_size, "==", self.win_size, "win size == %s" % self.win_size]
                )
    ...
    def build_win_scale(self):
        if self.win_scale != "*" and self.win_scale != "0":
            ...
            self.steps.append(
                [ws_off, "==", self.win_scale, "win scale == %s" % self.win_scale]
            )
    ...
    def build_eol_pad(self):
        ...
        while pad_left > 0:
            ...
            if self.ver == "4":
                ...
                self.steps.append(
                    [
                        "ip6[(40 + %s):%s]" % (pad_pos, cur_chunk_len),
                        op,
                        0,
                        "eol pad %s 0" % op,
                    ]
