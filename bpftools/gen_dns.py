...
ACCEPTABLE_CHARS = (
    set(string.printable) - set(string.whitespace) - set(string.punctuation)
)
...
""",
    ...
    parser.add_argument("domains", nargs="+", help="DNS domain patterns to match on")
    ...
    for domain in args.domains:
        ...
        for i, part in enumerate(parts):
            ...
            if matchstar:
                ...
                rule.append(
                    (True, [(len_is_char, chr(len(part)))] + [(True, c) for c in part])
                )
    ...
    def match_exact(rule, label, last=False):
        ...
        for is_char, b in rule:
            if is_char and b == "?":
                ...
                mask.append("\x00")
        mask = "".join(mask)
        s = "".join([is_char_b[1] for is_char_b in rule])
        print("    ; Match: %s %r  mask=%s" % (s.encode("utf-8").hex(), s, mask.encode("utf-8").hex()))
        off = 0
        while s:
            if len(s) >= 4:
                m, s = s[:4], s[4:]
                ...
                (mm,) = struct.unpack("!I", bytes(mm, "utf-8"))
                print("    ld [x + %i]" % off)
                if mm:
                    print("    or #0x%08x" % mm)
                    m |= mm
                ...
                (mm,) = struct.unpack("!H", bytes(mm, "utf-8"))
                print("    ldh [x + %i]" % off)
                if mm:
                    print("    or #0x%04x" % mm)
                    m |= mm
                ...
                (mm,) = struct.unpack("!B", bytes(mm, "utf-8"))
                print("    ldb [x + %i]" % off)
                if mm:
                    print("    or #0x%02x" % mm)
                    m |= mm
    ...
    if ipversion == 4:
        ...
        print(
            "    ld #%i" % (l3_off + 40 + 8 + 12)
        )  # 40B of ipv6 + 8B of udp + 12B of dns header
    ...
    for domain in args.domains:
        ...
        for part in domain.split("."):
            if part == "*":
                ...
                parts.append("".join(c if c in ACCEPTABLE_CHARS else "x" for c in part))
