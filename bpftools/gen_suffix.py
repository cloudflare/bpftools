...
This is the same as:
  ...
  %(prog)s 01 02 03
    ...
    parser.add_argument(
        "suffix", nargs="*", help="hex encoded suffix (spaces are ignored)"
    )
    ...
    print("    ld #len")
    print("    sub #%i" % (old_div(len(suffix_hex), 2),))
    print("    tax")
    print()

    suffix_hex = suffix_hex.rstrip("x")
    s = bytes.fromhex(suffix_hex).decode("utf-8")

    off = 0
    print("     ; matching %s" % (s.encode("utf-8").hex(),))
    print("     ;         %r" % (s,))
    while s:
        if len(s) >= 4:
            m, s = s[:4], s[4:]
            (m,) = struct.unpack("!I", bytes(m, "utf-8"))
            print("    ld [x + %i]" % off)
            print("    jneq #0x%08x, nomatch" % (m,))
            off += 4
        elif len(s) >= 2:
            m, s = s[:2], s[2:]
            (m,) = struct.unpack("!H", bytes(m, "utf-8"))
            print("    ldh [x + %i]" % off)
            print("    jneq #0x%04x, nomatch" % (m,))
            off += 2
        else:
            m, s = s[:1], s[1:]
            (m,) = struct.unpack("!B", bytes(m, "utf-8"))
            print("    ldb [x + %i]" % off)
