...
def find_binary(prefixes, name, args):
    ...
    print(
        prefix,
        "%r not found in your PATH nor LINUX_TOOLS_PATH" % (name,),
        file=sys.stderr,
    )
    os._exit(-2)
...
def bpf_compile(assembly):
    ...
    out, err = subprocess.Popen(
        [os.path.join(prefix, "bpf_asm")],
        stdin=subprocess.PIPE,
        stdout=subprocess.PIPE,
        stderr=subprocess.PIPE,
    ).communicate(bytes(assembly, "utf-8"))
    out = out.decode()
    err = err.decode()
...
def _looks_like_ip(l2, off):
    ...
    if len(l2) - off >= 40:
        ...
        if vertos & 0xF0 == 0x60 and pay_len + off + 40 == len(l2) and ttl > 0:
            return 6
    return None
...
def find_ip_offset(l2, max_off=40):
    ...
    for off in range(2, max_off + 2, 2):
...
def scrub_dns_name(data, ip_ihl, ip_hdr_off, entropy):
    ...
    while True:
        try:
            str_len_off = ord(
                data[dns_hdr_off + 12 + name_offset]
            )  # 12 is the offset inside the DNS packet
        except IndexError:
            print(
                "OOps, it seems this UDP packet is not properly formed DNS, break while True",
                file=sys.stderr,
            )
            break
        if str_len_off == 0:
        ...
        while idx < str_len_off:
            try:
                ...
                print(
                    "OOps, it seems this UDP packet is not properly formed DNS, break while idx",
                    file=sys.stderr,
                )
                break
            rtr = scrub_byte(
                rtr, ord("a"), ord("z") + 1, entropy[name_offset % len(entropy)]
            )
            rtr = scrub_byte(
                rtr, ord("A"), ord("Z") + 1, entropy[name_offset % len(entropy)]
            )
            rtr = scrub_byte(
                rtr, ord("0"), ord("9") + 1, entropy[name_offset % len(entropy)]
            )
...
def do_scrub(l2, ip_hdr_off):
    ...
    if ip_hdr_off not in (14, 16):
        raise Exception(
            "ip_hdr_off=%i Not ethernet, not sure how to scrub MACS" % ip_hdr_off
        )
    for i in range(ip_hdr_off - 2):
        data[i] = "\x00"
    ...
    if ipver & 0xF0 == 0x40:
        ...
        # Scrubbing IPs
        ip_ihl = (ipver & 0x0F) * 4
        for i in range(ip_hdr_off + 12, ip_hdr_off + 12 + 4 + 4, 1):
        ...
        # IPV6
        for i in range(ip_hdr_off + 8, ip_hdr_off + 8 + 16 + 16, 1):
            data[i] = scrub_byte(data[i], 0, 256, entropy[i % len(entropy)])
